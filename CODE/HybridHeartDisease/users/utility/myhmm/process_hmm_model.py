...
#         return cls(dict(zip(list, list(array))))
#
#     @property
#     def dict(self):
...
#
# def test_hmm_ram():
#     hml_rand = HiddenMarkovLayer.initialize(X, y)
...
#
#     df2 = pd.DataFrame(pd.Series(chains_rand).value_counts(), columns=['counts']).reset_index().rename(
#         columns={'index': 'chain'})
#     df2 = pd.merge(df2, df2['chain'].str.split('-', expand=True), left_index=True, right_index=True)
#
#     s = []
#     for i in range(T + 1):
#         s.append(df2.apply(lambda x: x[i] == y[i], axis=1))
#
